...
This project supports tracing via **Opik** and **Langfuse**.

### Langfuse Setup
To enable Langfuse tracing for the agent:
1. Ensure you have a running Langfuse instance (cloud or self-hosted).
2. Set the following environment variables:
   ```bash
   export LANGFUSE_PUBLIC_KEY="pk-lf-..."
   export LANGFUSE_SECRET_KEY="sk-lf-..."
   export LANGFUSE_BASE_URL="http://localhost:3000" # or your host
   ```

**⚠️ Important Version Note:**
If you are using a self-hosted Langfuse instance that is not on the absolute latest version, you must pin the python SDK version to match the server capabilities.
- **Current Pin:** `langfuse==3.10.5` is required for compatibility with our self-hosted instance.
- **Symptom of mismatch:** `pydantic.v1.error_wrappers.ValidationError: ... organization field required`.

### Opik Setup
1. Start your local Opik instance:
   ```bash
   cd ~/workspace/opik && ./opik.sh
   ```
2. The agent automatically instruments itself with Opik if the package is installed.
